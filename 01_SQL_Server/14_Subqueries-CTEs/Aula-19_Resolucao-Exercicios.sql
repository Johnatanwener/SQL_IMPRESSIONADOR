...
-- Lógica: Selecionar todos os funcionarios da tabela DimEmployee onde o departamento é igual ao departamento do funcionario Miguel Severino

-- 1. Descobrindo o departamento do funcionário Miguel Severino
SELECT DepartmentName FROM DimEmployee 
WHERE FirstName = 'Miguel' AND LastName = 'Severino'

-- 2. Descobrindo quais são os funcionários do departamento onde o funcionário Miguel Severino trabalha.
SELECT 
	*
FROM DimEmployee
...
-- 1. Descobrir a média salarial anual de clientes pessoas físicas = 
SELECT 
	AVG(YearlyIncome)
FROM DimCustomer
WHERE CustomerType = 'Person' -- Resposta: Média de $57.305,78

-- 2. Descobrir os clientes que recebem um salário acima da média.
SELECT
	CustomerKey,
	FirstName,
	LastName,
...
/* EXERCÍCIO 06
 A empresa implementou um programa de fidelização para clientes empresariais. Todos os clientes que compraram ou que comprarem mais de 3000 unidades de um mesmo produto receberão descontos em compras futuras. 
